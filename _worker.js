...
async function 获取前端页面(pathWithQuery, status = 200) {
	try {
		...
		// Safe server-side translation: translate HTML markup outside <script> and <style>
		const tokens = html.split(/(<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>)/i);
		for (let i = 0; i < tokens.length; i++) {
			if (!tokens[i].toLowerCase().startsWith('<script') && !tokens[i].toLowerCase().startsWith('<style')) {
				let t = tokens[i];
				for (let j = 0; j < sortedKeys.length; j++) {
					const k = sortedKeys[j];
					if (t.includes(k)) t = t.replaceAll(k, dict[k]);
				}
				tokens[i] = t;
			}
		}
